...
public class CampoMagnetico {
    ...
    private void criarInterface() {
        ...
        ((AbstractDocument) distanciaField.getDocument()).setDocumentFilter(new NumericFilter(0.01, 100));
        ((AbstractDocument) correnteField.getDocument()).setDocumentFilter(new NumericFilter(0.1, 1000));
        ((AbstractDocument) campoTerraField.getDocument()).setDocumentFilter(new NumericFilter(0.1, 100));
    ...
    private void resolver() {
        try {
            ...
            double campoTerra = Double.parseDouble(campoTerraField.getText()); // em μT

            final double mu0 = 4 * Math.PI * 1e-7; // T·m/A

            double Blinha = (mu0 * corrente) / (2 * Math.PI * distancia); // em tesla
            double BlinhaMicroT = Blinha * 1e6; // em μT
            ...
            outputArea.setText("Erro: Verifique os valores preenchidos.");
        }
    }

    // Filtro numérico com intervalo
